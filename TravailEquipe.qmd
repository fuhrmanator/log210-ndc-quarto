...
Ce sujet a été discuté dans le livre "*Team Geek*" [-@fitzpatrick_team_2012], écrit par Brian W. Fitzpatrick (ancien employé de Google) et Ben Collins-Sussman (développeur fondateur du système de contrôle de version Subversion, employé de Google).
...
Un exemple illustrant est le développement de code pour [Flash](https://fr.wikipedia.org/wiki/Adobe_Flash)\ {{< fa brands wikipedia-w >}}.
En 2011, la technologie Flash était déployée sur 50\ % des pages Web [@Boittiaux2016]. 
À cause de la technologie HTML5, cette technologie est maintenant désuète.
Pourtant beaucoup de développeurs et de développeuses Web ont investi énormément pour avoir une expertise en Flash à l'époque de sa popularité.
En moins de dix ans, ça ne vaut plus rien de mentionner cette compétence sur un CV.
...
Rappelez-les par l'acronyme HRC.
...
- Les membres de l'équipe (même la chef ou le chef) qui ont pris une mauvaise décision (technique ou autre) vont l'avouer. Elles et ils savent que les autres ne sont pas là pour les attaquer (il y a du respect).
...
Elle ou il sait que la qualité de son code n'équivaut pas à son estime de soi (cela n'est pas toujours facile!).
...
Sinon, la perte soudaine d'un(e) membre de l'équipe (pour une raison quelconque) peut engendrer de graves conséquences, voire l'arrêt du projet.
...
Un(e) membre de l'équipe peut s'absenter ou être moins disponible soudainement pour beaucoup de raisons.
...
Si une personne a choisi un cadriciel, par exemple, mais elle n'est plus là pour vous dire pourquoi, c'est une information perdue. 
...
::: {.callout-warning}
Dans un contexte universitaire, si un(e) membre de l'équipe quitte soudainement, il n'est pas facile de maintenir le même rythme.
Cependant, les enseignantes ou enseignants et et les auxiliaires de laboratoire s'attendront à ce que vous ayez pensé à un "plan B" pour faire face à ce contretemps.
...
Pour des raisons pédagogiques [@oakley2004turning], c'est l'enseignant ou enseignante qui décide la composition des équipes.
...
- savoir évaluer le besoin de la personne à encadrer dans l'équipe, pour bien quantifier l'aide dont vous allez lui fournir.
...
Un(e) bon(ne) mentor(e) doit pouvoir estimer le niveau de la personne et lui donner l’aide appropriée, ce qui n’est pas toujours facile.
...
Considérez les volets HRC lorsque vous vous trouvez dans une des situations suivantes (il s'agit des projets dans un contexte de cours, mais certains scénarios peuvent aussi arriver dans les équipes professionnelles):
...
- Un ou deux membres d'une équipe abandonnent le cours après les évaluations de mi-trimestre, par crainte d'échec de leur cours.
...
Le travail en équipe est essentiel pour une personne qui développe du logiciel.
Le respect, l'humilité et la confiance (HRC) sont les notions importantes du comportement humain lorsqu'on travaille en équipe.
