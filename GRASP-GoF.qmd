...
Les "variations ou l'instabilité" sont les calculateurs de taxes qui ne sont pas sous le contrôle des développeur(e)s du projet (ce sont des modules externes ayant chacun une API différente). Quant à l'" impact négatif sur les autres", il s'agit des modifications que les développeur(e)s auraient à faire sur la classe Vente chaque fois que l'on décide de supporter un autre calculateur de taxes (ou si l'API de ce dernier évolue). 
