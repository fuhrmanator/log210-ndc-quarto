...
Catégorie | Exemples
:----------|:----------
**Transactions métier**: Elles sont essentielles, commencez l'analyse par les transactions. | *Vente*, *Attaque*, *Réservation*, *Inscription*, *EmpruntVélo*
...
- Il y a des associations dérivées de la liste des associations courantes. Voir le tableau @Tableau_associations_courantes.
- En UML les associations sont représentées par des lignes entre classes.
  ...
  Vous pouvez en trouver plusieurs exemples dans la figure @fig-MDD-jeu-de-risk.
...
: Extrait du tableau 9.2 \faBook (liste d'associations courantes).{#Tableau_associations_courantes}
...
L'exemple à la @MDD-attribut-derive s'applique à la règle du jeu de Risk spécifiant qu'un joueur reçoit un certain nombre de renforts selon le nombre de pays occupés. La classe Joueur pourrait avoir un attribut dérivé `/nbPaysOccupés` qui est calculé selon le nombre de Pays contrôlés par le joueur.
...
La @MDD-CoursGroupeCoursDescription présente une classe de description pour le contexte de cours et groupe-cours.


::: {.callout-warning}
Attention de ne pas faire l'erreur naïve d'utiliser une classe de description simplement pour "décrire" une autre classe. Voir la @MDD-ErreurDescription pour un exemple.
:::
...
Dans l'exemple à la @MDD-classe-association, voici pourquoi il y a une classe d'association Occupation. 
...
- Composition, par exemple, la classe Continent qui groupe les Pays dans la @MDD-jeu-de-risk. Larman propose d'utiliser la composition lorsque:
...
- Attribut dérivé, par exemple, `/nbPaysOccupés` dans la classe Joueur est un attribut dérivé de l'association entre Joueur et Pays (@MDD-attribut-derive).
