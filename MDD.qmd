...
Par rétroingénierie, réalisez le modèle du domaine pour une fonctionnalité du logiciel Discord.
...
1. À l'aide de la liste de catégories de classes conceptuelles, identifiez les classes conceptuelles qui sont nécessaires pour réaliser la fonctionnalité de l'étape précédente.
...
**Lieu de la transaction, lieu du service** | *Salon textuel*
**Conteneurs** | *Serveur*, *Profil*, *Fichier*

::::

4. Identifiez les attributs de chaque classe conceptuelle qui sont nécessaires pour réaliser la fonctionnalité.

:::: {.callout-note collapse="true" icon="false"}

## Solution

```{.plantuml #fig-mdd-retroingenierie-discord-attributs caption="Attributs des classes conceptuelles identifées dans l'exercice."}
@startuml
!include ecriture.pumlinclude
scale 1
hide empty members
...
5. Identifiez les associations entre les classes conceptuelles ainsi que leurs cardinalités.

:::: {.callout-note collapse="true" icon="false"}

## Solution

```{.plantuml #fig-mdd-retroingenierie-discord-associations caption="Associations et cardinalités des classes conceptuelles identifées dans l'exercice."}
@startuml
!include ecriture.pumlinclude
scale 1
hide empty members

Utilisateur "1" -- "0..*" Message : rédige >
SalonTextuel "1" -- "0..*" Message : publie >
Profil "1" -- "1" Utilisateur : représente >
Utilisateur "1..*" -- "0..*" Serveur : est membre >
Utilisateur "1" -- "0..*" Serveur : est propriétaire >
Serveur "1" -- "0..*" SalonTextuel : héberge >
Message "1" -- "0..*" Fichier : contient >
Utilisateur "1..*" -- "0..*" SalonTextuel : est membre >
