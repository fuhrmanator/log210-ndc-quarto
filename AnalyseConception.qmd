...
```{.plantuml #fig-mddDes caption="Diagramme de *classes conceptuelles* décrivant (modélisant) le *problème* d'un jeu de dés [@craig_uml_2005, figure 1.3] avec des statistiques. Ceci est élaboré lors d'une activité d'analyse."}
@startuml
...
Les choix que font les ingénieures et ingénieurs dans un projet peuvent amener de la complexité circonstancielle.
...
La complexité circonstancielle (aussi appelée accidentelle) peut être gérée avec des technologies, par exemple les débogueurs, les patterns de conception (un Adaptateur pour les différentes bases de données), etc.
