...
Chaque équipe est libre à choisir parmi une liste d'exigences ce qu'elle veut réaliser dans chaque itération. 
...
Les deux premières itérations sont évaluées, mais les points pour les fonctionnalités réussites ne compte qu'à la dernière itération.
Cela a plusieurs avantages pédagogiques:

- réduction du risque d'échec, puisqu'on on a des rétroactions formelles à toutes les trois semaines environ.
- réduction de la facilité de faire du plagiat, puisqu'on doit construit en équipe un système tout au long du cours, et las éléments d'analyse et de conception (le MDD et les RDCU) doivent être cohérents avec le code (et les tests).
- possibilité de constater et de corriger tôt les dysfonctionnements dans une équipe (mais on doit être proactif et pret à confronter les problèmes, ce qui reste un défi culturel).
...
Pour la personne ayant déjà des connaissances de Java et des _patterns_ de conception de la Bande des quatre @GoF1994, il est recommandé d'apprendre les choses dans cet ordre:
