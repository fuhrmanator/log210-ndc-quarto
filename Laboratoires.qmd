...
Pour la personne ayant déjà des connaissances de Java et des patterns GoF, @GoF1994, il est recommandé d'apprendre les choses dans cet ordre:

- **JavaScript**: un tutoriel intéressant (et libre) se trouve sur [fr.javascript.info](https://fr.javascript.info/). Nous vous recommandons de contribuer à des [traductions en français sur GitHub](https://github.com/javascript-tutorial/fr.javascript.info#readme). 
...
- Les gabarits (*templates*) Pug (anciennement Jade): [Tutoriel (court)](https://www.youtube.com/watch?v=DSp9ExFw3Ig)\ {{< fa brands youtube >}}, [Tutoriel (plus complet)](https://www.youtube.com/watch?v=kt3cEjjkCZA)\ {{< fa brands youtube >}}.
- Bootstrap (mise en page avec CSS): [Tutoriel (attention, il faut appliquer les éléments dans les gabarits Pug)](https://www.youtube.com/watch?v=MrgGq3fob4o)\ {{< fa brands youtube >}}.
...
Contrairement à Java, les types des variables JavaScript peuvent changer dynamiquement.
...
Puisque chaque personne est libre de modifier les fichiers comme elle le veut, sans être bloquée par les autres contributeurs et contributrices, il est nécessaire de synchroniser et de fusionner les contributions de temps en temps. 
Cette synchronisation peut être plus ou moins compliquée, selon les travaux réalisés par chaque personne, par exemple lorsque deux entre elles ont modifié un même fichier sur leur ordinateur.
Une des forces de Git est l'ensemble des fonctionnalités permettant de gérer tous les cas de synchronisation.
Mais cet avantage peut aussi faire en sorte que Git soit compliqué à utiliser.
La @fig-git_fonctionnement présente un survol des concepts et des opérations de base de Git.
...
Apprendre tous les cas d'utilisation de Git serait long et inintéressant.
Il n'est pas nécessaire de tout comprendre pour commencer.

![Concepts et opérations de base de Git. ["Basic Git Concepts and Operations"](https://graphviz.org/Gallery/directed/git.html) de [Costa Shulyupin](https://gitlab.com/makelinux), utilisé sous [EPL](https://en.wikipedia.org/wiki/Eclipse_Public_License).](images/git.svg){#fig-git_fonctionnement}
...
Après l'évaluation à la fin de chaque itération, il est important de quantifier et de considérer la contribution de chacun et chacune au projet et de valider avec les responsabilités prévues dans le plan de l'itération.
...
Par exemple, on peut voir sur la @fig-GitinspectorOutput qu'Anne et Justin ont fait une contribution beaucoup plus importante que Francis et Mélanie. 
...
Une personne exclue de cette manière va probablement abandonner le cours, et vous perdrez définitivement un ou une membre de l'équipe.
...
- Est-ce que certaines personnes travaillent excessivement, sans donner la chance aux autres de contribuer?
...
Il y a aussi un risque sur le plan de la [Redondance des compétences dans l'équipe (*bus factor*)](#sec-Bus_Factor), surtout si une personne qui travaille beaucoup plus que les autres éprouve un problème d'épuisement à cause du fait qu'elle travaille trop.
