...
thin_space_pattern_ascii = '[;!%?]'
wide_space_pattern_ascii = '[%%:]'
...
local function space_high_punctuation_and_quotes(inlines)
    ...
    while inlines[i] do
        ...
        if inlines[i + 1] and
            ...
            (string.match(inlines[i + 1].text:sub( -1), all_space_pattern_ascii)) then
            -- print(i, "fixing:", inlines[i+1])
            -- inlines[i + 1].text = inlines[i + 1].text:sub(1, -2) ..
            --     thin_nbsp .. inlines[i + 1].text:sub( -1)
            inlines[i + 1].text = insert_nonbreaking_space_before_last_char(inlines[i + 1].text)
            -- already_spaced = true
        else
            -- ignore if nbsp is already in string
            if inlines[i].t == 'Str' and not (string.find(inlines[i].text, thin_nbsp) or string.find(inlines[i].text, nbsp)) then
                if string.len(inlines[i].text) > 1 and string.match(inlines[i].text:sub( -1), all_space_pattern_ascii) then
                    ...
                    -- insert thin_nbsp before last char
