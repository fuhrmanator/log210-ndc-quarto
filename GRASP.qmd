...
À une extrémité, il y a la mentalité de mettre presque zéro effort pour une conception, que l'on nomme "Hacking Cowboy". C'est le cas d'un hackathon (un marathon de programmation durant 24 ou 48 heures où il faut produire une solution rapidement), lors duquel, vous ne feriez pas un logiciel avec 10 patrons GoF, vous ne feriez pas non plus les diagrammes UML pour réfléchir à votre architecture. Vous savez aussi que le code produit lors d'un hackathon ne sera pas facile à maintenir. Le seul but de cette activité est de développer du code qui marche pour montrer une idée intéressante.
