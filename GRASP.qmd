...
À une extrémité, il y a la mentalité de mettre presque zéro effort pour une conception, que l'on nomme "Hacking Cowboy". C'est le cas d'un hackathon (un marathon de programmation durant 24 ou 48 heures où il faut produire une solution rapidement), lors duquel, vous ne feriez pas un logiciel avec 10 _patterns_ GoF, vous ne feriez pas non plus les diagrammes UML pour réfléchir à votre architecture. Vous savez aussi que le code produit lors d'un hackathon ne sera pas facile à maintenir. Le seul but de cette activité est de développer du code qui marche pour montrer une idée intéressante.
...
À l'autre extrémité du spectre de conception, on retrouve ce que l'on nomme "Cascade pure" où beaucoup d'effort a été déployé pour la conception.
...
Cependant, méfiez-vous des entreprises qui ne portent aucune attention à la conception (l'extrémité "hacking cowboy" du spectre), même si l'on vous dit que c'est "agile". <!-- Si vous négligez complètement la conception, vous pouvez peut-être produire du code qui fonctionne plus vite à court terme.
...
```{.plantuml #fig-GRASP_polymorphisme_ex caption='Classe "Bird" à laquelle on peut appliquer le principe GRASP Polymorhpisme.'}
@startuml
...
- Lesquelles des classes sur la @fig-GRASP_cohesion_ex1 ont de multiples responsabilités?
- Déterminez si les classes sur la @fig-GRASP_cohesion_ex2 ont une cohésion forte ou faible.

```{.plantuml #fig-GRASP_cohesion_ex1 caption="Lesquelles des classes ont de multiples responsabilités?"}
@startuml
skinparam style strictuml
skinparam defaultfontname Arial,Helvetica,sans-serif
class Jeu {
...
```{.plantuml #fig-GRASP_cohesion_ex2 caption="Lesquelles des classes ont une cohésion forte? Une cohésion faible?"}
@startuml
skinparam style strictuml
skinparam defaultfontname Arial,Helvetica,sans-serif
class Jeu {
