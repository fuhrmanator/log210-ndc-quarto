...
::: {#exr-GRASP_GOF_polymorphisme_createur_forte_cohesion_faible_couplage}

### GRASP Polymorphisme, créateur, fabrication pure, indirection, forte cohésion et faible couplage

Appliquez les GRASP polymorphisme, créateur, fabrication pure et indirection à la classe `SomeClass` pour améliorer sa cohésion et réduire son couplage.
Créez une interface `Parser` qui contient les méthodes pertinentes aux parsers et qui sera implémentée par ces derniers.
Ajoutez un parse pour chaque type de fichier traitée par la méthode `parsePart`.
Ajoutez une fabrique de `Parser` à qui `SomeClass` pourra déléguer la responsabilité de créer les objets `Parser`.
...
La création de l'interface `PartParser` est un exemple de polymorphisme qui permet de réduire le couplage entre `SomeClass` et les différents types de `PartParser`.
L'ajout de la fabrique `PartFactory` permet d'améliorer la cohésion de la classe `SomeClass` puisque ses responsabilités sont réduites.
La fabrique est un exemple du GRASP Indirection puisqu'elle élimine le couplage direct entre la classe `SomeClass` et les différents types de `PartParser`.
